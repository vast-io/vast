...
template <class T>
const record_type X<T>::layout = {{"value", data_to_type<T>{}}};
...
struct Narrow {
  ...
  template <class Inspector>
  friend auto inspect(Inspector& f, Narrow& a) {
    return f(a.value);
  }
...
const record_type Narrow::layout = {{"value", integer_type{}}};

TEST(narrowing) {
  auto x = Narrow{};
  auto r = record{{"value", integer{42}}};
  REQUIRE_EQUAL(convert(r, x), ec::no_error);
  CHECK_EQUAL(x.value, 42);
}
...
TEST(map to map) {
  ...
  REQUIRE_EQUAL(convert(layout, r, x), ec::no_error);
...
TEST(list of record to map) {
  ...
  REQUIRE_EQUAL(convert(layout, l1, x), ec::no_error);
  auto l2
    = list{record{{"outer", record{{"name", "z"}, {"value", integer{-42}}}}}};
  REQUIRE_EQUAL(convert(layout, l2, x), ec::no_error);
  CHECK_EQUAL(x.size(), 3u);
  ...
  REQUIRE_EQUAL(convert(layout, l2, x), ec::convert_error);
}
